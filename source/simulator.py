...
class FluiDNS():
    ...
    ##########################################################################################
    def parameters(self,SimTit,save_dir,pltop,isave,Re,Ri,Pr,bod,ene,buo,schema,pois_bound,bcxu,bcxv,bcyu,bcyv,bcxt,bcyt,rho,dt,vel,kn,A0,T0,omp,shape,x_pos,y_pos,length,width,radius,ptr,mot,yvel,amp,fn,st,mstar,zeta,omstar,theta):
        self.title = SimTit
        self.sdi = save_dir
        self.pto = pltop
        self.isave = isave
        ...
        self.part_track = ptr
        self.motion,self.y_vel = mot,yvel
        self.amp,self.fn,self.st = amp,fn,st
        self.mstar,self.omstar,self.zeta,self.theta = mstar,omstar,zeta,theta
...
#         ptrax,ptray = np.array([0]),np.array([6])
        ...
        for i in range(nt):
            ...
            if self.motion == 'VIV':
                y,ydot = af.VIV(ypos[-1],yvel[-1],cl,self.mstar,self.zeta,self.omstar,self.st,self.theta,self.dt)
