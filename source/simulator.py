...
class FluiDNS():
    ...
    ##########################################################################################
    def parameters(self,SimTit,save_dir,isave,idata,aeros,Re,Ri,Pr,bod,ene,buo,schema,pois_bound,bcxu,bcxv,bcyu,bcyv,bcxt,bcyt,rho,dt,vel,A0,T0,omp,shape,x_pos,y_pos,length,width,radius,mot,amp,fn,st):
        self.title = SimTit
        self.sdi = save_dir
    ...
    def declarations(self,ny,nx,restart,itr,save_dir,ene):
        ...
        if restart == 'new':
            self.u,self.v,self.p = np.zeros([ny,nx])+0.5,np.zeros([ny,nx]),np.zeros([ny,nx])+1
...
#             self.T = np.zeros([ny,nx])
    ...
    def solver(self,nt_st,nt_en):
        ...
        for i in range(nt):
            ...
            if self.ischeme == 'RK1':
                self.u,self.v,self.p,self.T = TIM.RK1(self.bod,self.ischeme,self.poi_b,self.ene,self.buo,self.bcxu,self.bcxv,self.bcyu,self.bcyv,self.bcxt,self.bcyt,                 self.ny,self.nx,self.dy,self.dx,self.rho,self.nu,self.Ri,self.Pr,self.dt,self.u,self.v,self.p,self.T,self.vel,t,self.A0,self.omp,eps,eps_in,self.T0)
                
            elif self.ischeme == 'CPM':
                self.u,self.v,self.p,self.T = TIM.CPM(self.bod,self.ischeme,self.poi_b,self.ene,self.buo,self.bcxu,self.bcxv,self.bcyu,self.bcyv,self.bcxt,self.bcyt,                                                self.ny,self.nx,self.dy,self.dx,self.rho,self.nu,self.Ri,self.Pr,self.dt                                                ,self.u,self.v,self.p,self.T,self.vel,t,self.A0,self.omp                 ,eps,eps_in,self.T0)
                
            elif self.ischeme == 'AB2':
                if i == 0:
                    un,vn,pn,Tn = TIM.RK1(self.bod,self.ischeme,self.poi_b,self.ene,self.buo,self.bcxu,self.bcxv,self.bcyu,self.bcyv,self.bcxt,self.bcyt,                                                self.ny,self.nx,self.dy,self.dx,self.rho,self.nu,self.Ri,self.Pr,self.dt                                                ,self.u,self.v,self.p,self.T,self.vel,t,self.A0,self.omp                                               ,eps,eps_in,self.T0)
                    
                else:
                    self.u,self.v,self.p,self.T,un,vn,pn,Tn = TIM.AB2(self.bod,self.ischeme,self.poi_b,self.ene,self.buo,self.bcxu,self.bcxv,self.bcyu,self.bcyv,self.bcxt,self.bcyt,                                                self.ny,self.nx,self.dy,self.dx,self.rho,self.nu,self.Ri,self.Pr,self.dt                                                ,self.u,un,self.v,vn,self.p,pn,self.T,Tn,self.vel,t,self.A0,self.omp                                               ,eps,eps_in,self.T0)
