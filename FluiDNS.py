...
if os.path.exists(save_dir) and res == 'new':
    ...
    if clean == 'y' or clean == 'yes':
        ...
        os.mkdir(save_dir)
        os.system('cp params.txt {}'.format('./'+str(SimTit)+'/_setup_.txt'))  
...
dog = parf[38]        # angle of gravitational vector
hflux = parf[39]
prints = int(parf[41])
...
sim.parameters(SimTit,save_dir,isave,idata,aeros,Re,Ri,Pr,bod,ene,buo,ischeme,pois_bound,bcxu,bcxv,bcyu,bcyv,bcxt,bcyt,rho,dt,u_vel,A0,T0,omp,shape,x_pos,y_pos,length,width,radius,mot,amp,fn,st,dog,hflux,probes,prints)
