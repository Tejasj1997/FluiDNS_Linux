...
if os.path.exists(save_dir) and res == 'new':
    ...
    if clean == 'y' or clean == 'yes':
        os.system("rm {}".format(save_dir))
        print("Directory cleaned. Making new one.")
        os.mkdir(save_dir)
        os.system("cp {} {}".format(work_dir+str('params.txt'),save_dir)
...
if parf[25] == 0:
    ...
    shape = 'square'
    x_pos,y_pos = parf[27],parf[28] - (width/2)
...
sim.parameters(SimTit,save_dir,pltop,isave,idata,aeros,Re,Ri,Pr,bod,ene,buo,ischeme,pois_bound,bcxu,bcxv,bcyu,bcyv,bcxt,bcyt,rho,dt,u_vel,kn,A0,T0,omp,shape,x_pos,y_pos,length,width,radius,mot,k,yvel,amp,fn,st)
